...
async def test_subscription_create_notification(settings):
    ...
    class TestConsumer(GenericAsyncAPIConsumer):
        ...
        def model_change(self,
                         instance: TestModelWithCustomPK,
                         action,
                         **kwargs) -> Dict:
    ...
    # subscribe
    subscription_id = 1
    await communicator.send_json_to( { 'action': 'subscribe_to_all_changes', 'request_id': subscription_id } )

    # create an instance
    created_instance = await database_sync_to_async(TestModelWithCustomPK.objects.create)(name='some_unique_name')
