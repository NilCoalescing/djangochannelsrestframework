...
class DetachedTaskConsumerMixin:
    _tasks_pending_attachment = []

    async def await_many_dispatch(self, consumer_callables, dispatch):
        """
        ...
        try:
            while True:
                ...
                await asyncio.wait(
                    tasks + detached_tasks, return_when=asyncio.FIRST_COMPLETED
                )
                ...
                # cleanup detached tasks
                _detached_tasks = list(detached_tasks)
                for task in _detached_tasks:
                    if task.done():
                        task.result()
                        detached_tasks.remove(task)

                detached_tasks += self._tasks_pending_attachment
                self._tasks_pending_attachment = []
        finally:
    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            if self.channel_layer is not None:
                await self.await_many_dispatch(
                    [receive, self.channel_receive], self.dispatch
                )
            else:
                await self.await_many_dispatch([receive], self.dispatch)
        except StopConsumer:
            # Exit cleanly
            pass
